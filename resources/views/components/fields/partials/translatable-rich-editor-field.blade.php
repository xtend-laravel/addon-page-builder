<div
    x-data="richEditorFormComponent({
        state: $wire.{{ $applyStateBindingModifiers('entangle(\'' . $getStatePath() . '.' . $locale . '\')') }},
    })"
    ...
    x-on:trix-attachment-add="
        ...
        $wire.upload(
            `componentFileAttachments.{{ $getStatePath() }}.{{ $locale }}`,
            attachment.file,
            () => {
                $wire
                    .getComponentFileAttachmentUrl('{{ $getStatePath() }}.{{ $locale }}')
                    .then((url) => {
...
>
    @unless ($isDisabled())
        <input id="trix-value-{{ $getId() }}.{{ $locale }}" type="hidden" />

        <trix-toolbar
            id="trix-toolbar-{{ $getId() }}.{{ $locale }}"
            @class([
        ...
        <trix-editor
            ...
            id="{{ $getId() }}.{{ $locale }}"
            input="trix-value-{{ $getId() }}.{{ $locale }}"
            placeholder="{{ $getPlaceholder() }}"
            toolbar="trix-toolbar-{{ $getId() }}.{{ $locale }}"
            x-ref="trix"
