...
>
    @if ($getTranslatable())
        <x-hub::translatable>
            @include('xtend-lunar-page-builder::components.fields.partials.translatable-rich-editor-field' , ['locale' => $getDefaultLanguage()->code])
            @foreach($getLanguages()->filter(fn ($lang) => !$lang->default) as $language)
               <x-slot :name="$language['code']">
                   @include('xtend-lunar-page-builder::components.fields.partials.translatable-rich-editor-field', ['locale' => $language->code])
               </x-slot>
            @endforeach
        </x-hub::translatable>
    @else
